<?php if ($resume) { ?>
  ...
  * { font-family: DejaVu Sans, sans-serif; direction: rtl;}
  <?php if (adminSession('admin_language_dir') == 'ltr') { ?>
  table td, table tr, table tr td h2, table tr td p {direction: rtl;}
  <?php } ?>
  </style>
  <table>
    <tr>
      ...
      <td colspan="2">
        ...
        <div>
          ...
          <?php foreach ($resume['skills'] as $skill) { ?>
            <li><h3><?php echo esc_output($skill['title'], 'html'); ?> (<?php echo esc_output($skill['proficiency'], 'html'); ?>)</h3></li>
          <?php } ?>
          </ul>
        </div>
        <?php } else { ?>
