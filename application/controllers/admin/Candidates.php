...
use Dompdf\Dompdf;
// use application\models\AdminCandidateModel;

require_once  "system/libraries/resume-parser/PdfParser.php";

// recruit\script\system\libraries\resume-parser\inc\PdfParser.php
class Candidates extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model("AdminCandidateModel");
        // $this->load->library('resume-parser/inc/pdfparser');
    ...
    public function parseResume($url)
    {
        $url_name = "http://127.0.0.1:5000?file=".$url;
    ...
    public function createUserResume($resume){
    ...
    public function addCandidateByResume()
    {
        $config['upload_path']          = 'system/uploads/';
        $config['allowed_types']        = 'pdf|docx|doc';
        $this->load->library('upload', $config);

        $this->upload->do_upload('resumes');
        $data = $this->upload->data();
        print_r($data);
        $url = $data["full_path"];
        // $url = "https://res.cloudinary.com/dhtgqzkse/image/upload/v1681132590/personal/Resume-AbdulBateen-Jolaoshoo_djmeh0.pdf";
        $resume =  json_decode($this->parseResume($url));
        print_r("<br><b>");
        print_r($resume);
        die();

        $admin = new AdminCandidateModel();
        if ($admin->getCandidate("email", $resume->email)) {
            print_r("<br>");
            print_r($admin->getCandidate("email", $resume->email)['email']);
            print_r("<br>" . "User exists already");
        }
